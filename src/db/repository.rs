use sqlx::{PgPool, QueryBuilder, Postgres, FromRow, Row};
use uuid::Uuid;
...
where
    T: Send + Sync + Unpin + for<'r> FromRow<'r, sqlx::postgres::PgRow> + Serialize + DeserializeOwned + 'static,
{
    ...
    pub async fn list(&self, filter: Option<&str>, sort: Option<&str>, limit: Option<i64>, offset: Option<i64>) -> Result<Vec<T>> {
        let mut query_builder: QueryBuilder<Postgres> = QueryBuilder::new(format!("SELECT * FROM {}", self.table_name));
    ...
    pub async fn count(&self, filter: Option<&str>) -> Result<i64> {
        let query_str = match filter {
            Some(filter_clause) => format!("SELECT COUNT(*) AS count FROM {} WHERE {}", self.table_name, filter_clause),
    ...
    /// Save a new entity
    pub async fn create(&self, entity: &T) -> Result<i64> {
        ...
        let uuid = json.get("uuid").and_then(|v| v.as_str()).ok_or_else(|| Error::Entity("Missing uuid".to_string()))?;
        let path = json.get("path").and_then(|v| v.as_str()).ok_or_else(|| Error::Entity("Missing path".to_string()))?;
        ...
        let created_at = json.get("created_at").and_then(|v| v.as_str()).ok_or_else(|| Error::Entity("Missing created_at".to_string()))?;
        let updated_at = json.get("updated_at").and_then(|v| v.as_str()).ok_or_else(|| Error::Entity("Missing updated_at".to_string()))?;
        let created_by = json.get("created_by").and_then(|v| v.as_i64());
        let updated_by = json.get("updated_by").and_then(|v| v.as_i64());
        let published = json.get("published").and_then(|v| v.as_bool()).unwrap_or(false);
        let version = json.get("version").and_then(|v| v.as_i64()).unwrap_or(1);
        
        // Get custom fields
        let custom_fields = json.get("custom_fields").cloned().unwrap_or(serde_json::json!({}));
    ...
    pub async fn update(&self, id: i64, entity: &T) -> Result<()> {
    ...
    async fn create_version_snapshot(&self, _id: i64, _entity: &T) -> Result<()> {
