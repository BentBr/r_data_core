...
pub async fn run_migrations(pool: &PgPool) -> Result<()> {
    ...
    type MigrationFn = for<'a> fn(&'a PgPool) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + 'a>>;
    ...
    let migration_fns: Vec<(&str, MigrationFn)> = vec![
        ...
        ("003_workflow_tables", |pool| Box::pin(apply_workflow_tables(pool))),
        ("004_refresh_schema", |pool| Box::pin(schema::refresh_classes_schema(pool))),
    ];

    // Check which migrations have already been applied
    let applied_migrations: Vec<String> = query_as::<_, AppliedMigration>("SELECT name FROM migrations")
        .fetch_all(pool)
        .await
        .map_err(|e| Error::Database(e))?
        .into_iter()
