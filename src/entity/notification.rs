...
impl Notification {
    ...
    pub fn with_recipient_id(mut self, recipient_id: i64) -> Self {
        ...
        match self.notification_type {
            NotificationType::Email => {
                if self.recipient_email.is_none() && self.recipient_id.is_none() {
                    return Err(Error::Validation("Email notification requires either recipient_email or recipient_id".to_string()));
                }
            },
            NotificationType::SMS => {
                if self.recipient_phone.is_none() && self.recipient_id.is_none() {
                    return Err(Error::Validation("SMS notification requires either recipient_phone or recipient_id".to_string()));
                }
            },
            NotificationType::System => {
                if self.recipient_id.is_none() {
                    return Err(Error::Validation("System notification requires recipient_id".to_string()));
                }
            },
            _ => {}
        }
