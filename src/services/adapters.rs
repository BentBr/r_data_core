...
impl ClassDefinitionRepositoryTrait for ClassDefinitionRepositoryAdapter {
    ...
    async fn get_by_uuid(&self, uuid: &Uuid) -> Result<Option<ClassDefinition>> {
        log::debug!("ClassDefinitionRepositoryAdapter::get_by_uuid called with uuid: {}", uuid);
        self.inner.get_by_uuid(uuid).await
    }
    
    async fn get_by_entity_type(&self, entity_type: &str) -> Result<Option<ClassDefinition>> {
        log::debug!("ClassDefinitionRepositoryAdapter::get_by_entity_type called with entity_type: {}", entity_type);
    ...
    async fn create(&self, definition: &ClassDefinition) -> Result<Uuid> {
        ...
        log::debug!("Definition schema properties: {:?}", definition.schema.properties);
    ...
    async fn update(&self, uuid: &Uuid, definition: &ClassDefinition) -> Result<()> {
        log::debug!("ClassDefinitionRepositoryAdapter::update called with uuid: {}", uuid);
        self.inner.update(uuid, definition).await
    }
    
    async fn delete(&self, uuid: &Uuid) -> Result<()> {
        log::debug!("ClassDefinitionRepositoryAdapter::delete called with uuid: {}", uuid);
    ...
    async fn update_entity_view_for_class_definition(&self, class_definition: &ClassDefinition) -> Result<()> {
        log::debug!("ClassDefinitionRepositoryAdapter::update_entity_view_for_class_definition called");
        self.inner.update_entity_view_for_class_definition(class_definition).await
    }
    
    async fn check_view_exists(&self, view_name: &str) -> Result<bool> {
        log::debug!("ClassDefinitionRepositoryAdapter::check_view_exists called with view_name: {}", view_name);
    ...
    async fn count_view_records(&self, view_name: &str) -> Result<i64> {
        log::debug!("ClassDefinitionRepositoryAdapter::count_view_records called with view_name: {}", view_name);
