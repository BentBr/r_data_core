...
// Re-export all config types from the core
pub use r_data_core_core::config::{
    ApiConfig, AppConfig, CacheConfig, DatabaseConfig, LogConfig, MaintenanceConfig, QueueConfig,
    WorkflowConfig, WorkerConfig,
};
...
pub fn load_app_config() -> Result<AppConfig> {
    ...
    let database = DatabaseConfig {
        connection_string: env::var("DATABASE_URL")
            .map_err(|_| r_data_core_core::error::Error::Config("DATABASE_URL not set".to_string()))?,
    ...
    let api = ApiConfig {
        ...
        jwt_secret: env::var("JWT_SECRET")
            .map_err(|_| r_data_core_core::error::Error::Config("JWT_SECRET not set".to_string()))?,
...
pub fn load_worker_config() -> Result<WorkerConfig> {
    ...
    let interval_str = env::var("JOB_QUEUE_UPDATE_INTERVAL")
        .map_err(|_| r_data_core_core::error::Error::Config("JOB_QUEUE_UPDATE_INTERVAL not set".to_string()))?;
    ...
    let database = DatabaseConfig {
        connection_string: env::var("WORKER_DATABASE_URL")
            .map_err(|_| r_data_core_core::error::Error::Config("WORKER_DATABASE_URL not set".to_string()))?,
...
pub fn load_maintenance_config() -> Result<MaintenanceConfig> {
    ...
    let cron = env::var("MAINTENANCE_CRON")
        .map_err(|_| r_data_core_core::error::Error::Config("MAINTENANCE_CRON not set".to_string()))?;
    // Validate cron expression using the same logic as cron preview
            utils::validate_cron(&cron)
        .map_err(|e| r_data_core_core::error::Error::Config(format!("Invalid MAINTENANCE_CRON '{cron}': {e}")))?;

    let version_purger_cron = env::var("VERSION_PURGER_CRON")
        .map_err(|_| r_data_core_core::error::Error::Config("VERSION_PURGER_CRON not set".to_string()))?;
    ...
    let connection_string = env::var("MAINTENANCE_DATABASE_URL")
        .map_err(|_| r_data_core_core::error::Error::Config("MAINTENANCE_DATABASE_URL not set".to_string()))?;
    ...
    let cache = get_cache_config();
    let redis_url =
        env::var("REDIS_URL").map_err(|_| r_data_core_core::error::Error::Config("REDIS_URL not set".to_string()))?;
...
fn get_queue_config() -> Result<QueueConfig> {
    let config = QueueConfig {
        redis_url: env::var("REDIS_URL")
            .map_err(|_| r_data_core_core::error::Error::Config("REDIS_URL not set".to_string()))?,
