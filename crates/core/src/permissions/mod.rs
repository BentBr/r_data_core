...
pub enum PermissionScope {
    Entity { entity_type: String, entity_uuid: Option<Uuid> },
    Workflow { workflow_uuid: Option<Uuid> },
    System,
}
...
impl<R: PermissionRepository> PermissionChecker for DefaultPermissionService<R> {
    async fn is_allowed(
        ...
        let perms = self.repository.get_permissions_for_roles(&ctx.roles).await?;
        let matches_action = |p: &Permission| &p.permission_type == action;
        let within_scope = |p: &Permission| {
            matches!(scope, PermissionScope::System | PermissionScope::Workflow { .. })
                || {
