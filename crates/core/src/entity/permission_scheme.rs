...
impl FromRow<'_, PgRow> for PermissionScheme {
    fn from_row(row: &PgRow) -> std::result::Result<Self, sqlx::Error> {
        ...
        let role_permissions: HashMap<String, Vec<Permission>> = match row
            .try_get::<serde_json::Value, _>("role_permissions")
        {
            Ok(json) => serde_json::from_value(json).unwrap_or_else(|_| HashMap::new()),
            Err(_) => HashMap::new(),
        };
