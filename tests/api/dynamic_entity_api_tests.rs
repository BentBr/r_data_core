...
mod dynamic_entity_api_tests {
    ...
    // Helper to create a test class definition for user entity
    async fn create_user_class_definition(db_pool: &sqlx::PgPool) -> Result<Uuid> {
        // Create a simple class definition for testing
        let mut class_def = ClassDefinition::default();
        class_def.entity_type = "user".to_string();
        class_def.display_name = "Test User".to_string();
        class_def.description = Some("Test description for User".to_string());
    ...
    > {
        ...
        let class_definition_service = ClassDefinitionService::new(
            Arc::new(r_data_core::entity::class::repository::ClassDefinitionRepository::new(
                db_pool.clone(),
            )),
        );
    ...
    async fn test_get_users_api() -> Result<()> {
        ...
        // Create class definition for user entity
        let _class_uuid = create_user_class_definition(&db_pool).await?;
