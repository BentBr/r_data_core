...
export function useTranslations() {
  ...
  const initTranslations = async () => {
    if (Object.keys(translations.en).length === 0) {
      await Promise.all(
        AVAILABLE_LANGUAGES.map(lang => loadTranslation(lang))
      )
    }
  }
  ...
  const translateError = (errorMessage: string): string => {
    ...
    if (lowerMessage.includes('invalid credentials') || lowerMessage.includes('invalid username') || lowerMessage.includes('invalid password')) {
