...
export function getStepStats(step: DslStep, t: TranslateFn): StepStats {
    const stats: StepStats = [
        { label: t('workflows.dsl.stats.input_mappings'), value: String(mappingCount(step.from.mapping)) },
        { label: t('workflows.dsl.stats.output_mappings'), value: String(mappingCount(step.to.mapping)) },
    ]
