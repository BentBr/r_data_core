<template>
    <div class="dsl-config">
        ...
        <v-skeleton-loader v-if="loading" type="list-item-two-line" class="mb-2" />
        <v-expansion-panels v-model="openPanels" multiple>
            <v-expansion-panel v-for="(step, idx) in stepsLocal" :key="idx">
                <v-expansion-panel-title>
                    {{ t('workflows.dsl.step_label', { n: String(idx + 1) }) }} — from: {{ step.from?.type || '-' }}, transform: {{ step.transform?.type || '-' }}, to: {{ step.to?.type || '-' }}
                    <v-spacer />
                    <v-btn icon="mdi-delete" size="x-small" variant="text" color="error" @click.stop="removeStep(idx)" />
                </v-expansion-panel-title>
                <v-expansion-panel-text>
                    <div class="mb-4">
                        ...
                        <template v-else-if="step.from.type === 'entity'">
                            <v-text-field v-model="(step.from as any).entity_definition" :label="t('workflows.dsl.entity_definition')" density="comfortable" @update:model-value="emitChange" />
                            <div class="d-flex ga-2">
                                <v-text-field v-model="(step.from as any).filter.field" :label="t('workflows.dsl.filter_field')" density="comfortable" @update:model-value="emitChange" />
                    ...
                    <div class="mb-4">
                        ...
                        <v-select v-model="transformType" :items="transformTypes" :label="t('workflows.dsl.transform_type')" density="comfortable" class="mb-2" @update:model-value="applyTransformType(step)" />
                        <template v-if="transformType === 'arithmetic'">
                            <div class="d-flex ga-2">
                                <v-text-field v-model="(step.transform as any).target" :label="t('workflows.dsl.target')" density="comfortable" @update:model-value="emitChange" />
                                <v-select v-model="(step.transform as any).op" :items="ops" :label="t('workflows.dsl.op')" density="comfortable" class="flex-0" style="max-width: 160px" @update:model-value="emitChange" />
                            </div>
                            <div class="d-flex ga-2">
                                <v-select v-model="left.kind" :items="operandKinds" :label="t('workflows.dsl.left_kind')" density="comfortable" class="flex-0" style="max-width: 180px" @update:model-value="syncLeft(step)" />
                                <v-text-field v-if="left.kind==='field'" v-model="left.field" :label="t('workflows.dsl.left_field')" density="comfortable" @update:model-value="syncLeft(step)" />
                                <v-text-field v-else v-model.number="left.value" :label="t('workflows.dsl.left_value')" type="number" density="comfortable" @update:model-value="syncLeft(step)" />
                            </div>
                            <div class="d-flex ga-2">
                                <v-select v-model="right.kind" :items="operandKinds" :label="t('workflows.dsl.right_kind')" density="comfortable" class="flex-0" style="max-width: 180px" @update:model-value="syncRight(step)" />
                                <v-text-field v-if="right.kind==='field'" v-model="right.field" :label="t('workflows.dsl.right_field')" density="comfortable" @update:model-value="syncRight(step)" />
                                <v-text-field v-else v-model.number="right.value" :label="t('workflows.dsl.right_value')" type="number" density="comfortable" @update:model-value="syncRight(step)" />
                            </div>
                        </template>
                        <template v-else-if="transformType === 'concat'">
                            <div class="d-flex ga-2">
                                <v-text-field v-model="(step.transform as any).target" :label="t('workflows.dsl.target')" density="comfortable" @update:model-value="emitChange" />
                                <v-text-field v-model="(step.transform as any).separator" :label="t('workflows.dsl.separator')" density="comfortable" @update:model-value="emitChange" />
                            </div>
                            <div class="d-flex ga-2">
                                <v-select v-model="leftConcat.kind" :items="stringOperandKinds" :label="t('workflows.dsl.left_kind')" density="comfortable" class="flex-0" style="max-width: 200px" @update:model-value="syncLeftConcat(step)" />
                                <v-text-field v-if="leftConcat.kind==='field'" v-model="leftConcat.field" :label="t('workflows.dsl.left_field')" density="comfortable" @update:model-value="syncLeftConcat(step)" />
                                <v-text-field v-else v-model="leftConcat.value" :label="t('workflows.dsl.left_value')" density="comfortable" @update:model-value="syncLeftConcat(step)" />
                            </div>
                            <div class="d-flex ga-2">
                                <v-select v-model="rightConcat.kind" :items="stringOperandKinds" :label="t('workflows.dsl.right_kind')" density="comfortable" class="flex-0" style="max-width: 200px" @update:model-value="syncRightConcat(step)" />
                                <v-text-field v-if="rightConcat.kind==='field'" v-model="rightConcat.field" :label="t('workflows.dsl.right_field')" density="comfortable" @update:model-value="syncRightConcat(step)" />
                                <v-text-field v-else v-model="rightConcat.value" :label="t('workflows.dsl.right_value')" density="comfortable" @update:model-value="syncRightConcat(step)" />
                            </div>
                        </template>
                    </div>
...
<script setup lang="ts">
    ...
    function syncLeftConcat(step: DslStep) {
        if ((step.transform as any).type !== 'concat') return
